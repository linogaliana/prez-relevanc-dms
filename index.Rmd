...
* Au sein d'une classe de produit (ex: lasagnes), énormément d'__hétérogénéité__ de *"qualité"*
...
* Besoin d'__enrichissements multiples__:
    + Sur la dimension `géographique` ;
    + Sur la dimension `produit`

--
* Méthode des __appariements flous__: 
    + Compenser absence d'identifiants pour appariements exacts
    + Noms produits
    + Adresse magasins

--
* __Catégorisation__ pour réduire le nombre de paires
    + COICOP
    + Département
...
* __EAN__ est un identifiant produit unique: appariement exact entre `RelevanC` et `OpenFood`
* Pour la calorie: permet d'enrichir __46% des données__ `RelevanC`
...
* *Open-source*, possibilité d'avoir des instances internes (comme sur SSP-Cloud ou instance Kube interne)
* Livré avec interface graphique `Kibana`:
    + Pratique pour *monitoring*
    + Mieux vaut passer par `Python` pour __indexation__ & __requêtage__
...
La proximité de caractère (__proximité syntaxique__) n'implique pas nécessairement une
__proximité sémantique__.
...
* Test d'un plongement de mots: associer à un libellé un vecteur dans $\mathbb{R}^N$ dont la position dans cette espace reflèterait un concept de produit
...
Les désaccords au sens de la distance siamoise entre le match Pipeline Elastic et le match Siamois repère les cas problématiques.
```{r}
...
* Calories: XXX% des produits enrichis (XXX% du CA)
...
---
# Bilan de l'appariement produit
## Distribution des distances textuelles
...
---
# Appariement Magasin: Siretisation
1. Appariement Flou: à l'aide de la librarie python `recordLinkages` ( $\approx$ IRI )
2. Complétée, pour les magasins au plus gros CA (e.g. Géant Casino) par des recherches ad-hoc via Elastic (via critères additionels, comme l'utilisation du SIREN de l'U.L. "Casino Distribution France")
